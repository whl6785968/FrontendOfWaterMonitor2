...
const state = {
  currCount: 0,
  stomp: Stomp.over(new SockJS("http://localhost:8088/ws/pullMsg")),
  noticeCount: 0,
  bdStatus: 0,
  dyn_is_connect: false
}

const mutations = {
  setDynIsConnect(state,payload){
    state.dyn_is_connect = true
  }
}
...
//  state.stomp = Stomp.over(new SockJS("http://localhost:8088/ws/pullMsg"))
  ...
  connectForBd({commit}){
    state.stomp.connect({},frame => {
      state.stomp.subscribe("/topic/getNewDataForBD",message => {
        state.bdStatus += 1
      })
    })
  },
  closeForBd({ commit }){
    if(state.stomp2 != null){
      state.stomp2.disconnect()
    }
  },
  closeConnect({ commit }){
    if(state.stomp != null){
